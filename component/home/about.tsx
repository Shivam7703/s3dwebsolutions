...
export default function AboutSection() {
  ...
  // Heading sabsay pehle enter karegi aur sabsay pehle upar exit karegi
  ...
  // Paragraph heading se thoda sa peeche chalega
  ...
  // Har card alag timeline par upar fade out hoga (Staggered effect)
  ...
  return (
    ...
    >
      <div className="max-w-6xl w-full flex flex-col gap-16 md:gap-24">
        ...
        {/* Top Header Block split into independent motion elements */}
        <div className="flex flex-col md:flex-row items-start justify-between gap-8 md:gap-16">
          ...
          >
            ...
            <h2 className={`mt-3 text-4xl md:text-5xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-zinc-800'}`}>
East Or West , We Are The Best For Whole Web Solutions            </h2>
          </motion.div>
          ...
          >
            <p className={`text-lg leading-relaxed md:mt-5 ${isDark ? 'text-zinc-100' : 'text-zinc-700'}`}>
        ...
        {/* Feature Grid Block where each card tracks its own scroll logic */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {cards.map((card, index) => (
            <motion.div
              ...
              className={`group relative p-8 rounded-3xl border transition-all duration-300 backdrop-blur-sm 
                ${isDark 
                  ? "bg-zinc-900/40 border-zinc-800/80 hover:border-violet-500/50 hover:bg-zinc-900/60 shadow-[0_0_30px_rgba(0,0,0,0.3)]" 
                  : "bg-white/40 border-zinc-200/80 hover:border-orange-400/50 hover:bg-white/90 shadow-[0_10px_30px_rgba(0,0,0,0.04)]"
                }`}
            >
              {/* Premium Glow Overlay */}
              <div className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-lg pointer-events-none -z-10
                ${isDark ? "bg-violet-500/10" : "bg-orange-500/10"}`} 
              />

              {/* Icon */}
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white  border transition-transform duration-300 group-hover:scale-110 bg-linear-to-br 
                ${isDark
          ...
          : "from-orange-500 to-red-500 "
              ...
              {/* Card Text */}
              <h3 className={`mt-6 text-xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-zinc-800'}`}>
                {card.title}
              </h3>
